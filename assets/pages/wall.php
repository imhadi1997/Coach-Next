...
if($post['uid']==$user['id']){
  ...
  <ul class="dropdown-menu" aria-labelledby="option<?=$post['id']?>">
    ...
    <?php
                ...
                <img src="assets/images/posts/<?=$post['post_img']?>" loading=lazy class="" style="height:450px;overflow:hidden;background-position: center;object-fit:cover;"alt="...">
    ...
    <?php
                ...
                <div class="input-group p-2 <?=$post['post_text']?'border-top':''?>">
                        ...
                        <input type="text" class="form-control rounded-0 border-0 comment-input" placeholder="say something.."
                            ...
                            <button class="btn btn-outline-danger rounded-0 border-0 add-comment" data-page='wall' data-cs="comment-section<?=$post['id']?>" data-post-id="<?=$post['id']?>" type="button"
